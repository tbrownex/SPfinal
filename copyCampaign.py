...
from postAd_HTML import postAd
from config import getBaseURL, getAdPath
...
def copyCampaign(orgID, masterIDs, numCopies):
    ...
    for campaignID in masterIDs:
        newIDs = processMaster(orgID, campaignID, numCopies)
    return newIDs

def processMaster(orgID, campaignID, numCopies):
    master = getCampaign(orgID, campaignID)
    master = master['campaigns'][0]
    name = master['name']
    payload = formatPayload(master)
    # Now create new campaigns for new Org
...
def processAddresses(orgID, newIDs):
    ...
    for idx, campaign in enumerate(newIDs):
